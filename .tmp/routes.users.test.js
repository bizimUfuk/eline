...
describe('GET /api/v1/users/:id', () => {
  it('should respond with a single user', (done) => {
    ...
    .end((err, res) => {
      ...
      res.body.status.should.eql('success');
      // the JSON response body should have a
      // key-value pair of {"data": [1 user objects]}
  ...
  it('should respond with a success message along with a single user that was deleted', (done) => {
    ...
    .then((users) => {
      ...
      .end((err, res) => {
        ...
        knex('users').select('*')
        .then((updatedUsers) => {
          // ensure the user count decreased
          updatedUsers.length.should.eql(lengthBeforeDelete - 1);
          // ensure the user deleted doesnt exist anymore
          updatedUsers.forEach((usr) => usr.id.should.not.eql(userObject.id))
          done();
        });
      });
    });
